...
export const SocialShareButton: React.FC<SocialShareButtonProps> = ({
  ...
  const platformConfig = {
    ...
    copy: {
      ...
      color: justCopied ? 'hover:bg-green-500 hover:text-white' : 'hover:bg-cyan-500 hover:text-white',
      bgColor: justCopied ? 'bg-green-500' : 'bg-cyan-500'
    }
  };
  ...
  const getVariantClasses = () => {
    switch (variant) {
      ...
      case 'minimal':
        return `text-cyan-300 ${config.color} ${sizeClasses[size]} transition-colors duration-200`;
      case 'icon':
      default:
        return `bg-black/20 border border-cyan-400/30 text-cyan-300 ${config.color} ${sizeClasses[size]} rounded-lg transition-all duration-200 hover:border-cyan-400 hover:scale-105 active:scale-95 backdrop-blur-sm`;
    }
  };
  ...
  return (
    ...
    >
      ...
      {(showLabel || variant === 'button') && (
        <span className={variant === 'icon' ? 'sr-only' : ''}>{displayLabel}</span>
      )}
    </button>
  );
};
